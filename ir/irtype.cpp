...
IrTypeValueof::IrTypeValueof(Type* dt, LLType* lt)
: IrType(dt, lt)
{
}
...
IrTypeValueof* IrTypeValueof::get(Type* dt)
{
    assert(!dt->ctype);
    assert((dt->ty == Tvalueof) && "not valueof type");

    auto pointerType = llvm::cast<llvm::PointerType>(DtoType(dt->nextOf())); // semi-HACK

    IrTypeValueof* t = new IrTypeValueof(dt, pointerType->getElementType());
    dt->ctype = t;
    return t;
}
